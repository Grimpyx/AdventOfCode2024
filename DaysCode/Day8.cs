            ...
            foreach (var nodeSet in antennaNodes)
            {
                ...
                foreach (var thisNode in nodeSet.Value)
                {
                    ...
                    foreach (var otherNode in nodeSet.Value)
                    {
                        ...
                        // For part two, we only need to modify what will be the new antinodes
                        // Essentially, instead of skipping over the antenna, we place an
                        // antinode at the antenna, and also at all positions beyond it the same 
                        // distance away. See the puzzle
                        // Calculate the position of the antinode
                        Int2 diff = otherNode.Position - thisNode.Position;
                        int counter = 1;
                        Int2 newAntinodePos = Int2.Zero;
                        do
                        {
                            newAntinodePos = thisNode.Position + counter * diff;

                            // Add them to the antinodes
                            antinodes.Add(newAntinodePos);
                            uniqueAntinodes.Add(newAntinodePos);
                        ...
                        while (charMap.IsInside(newAntinodePos + diff));
